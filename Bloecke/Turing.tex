...
\begin{frame}{Aufgabe}
	...
	\begin{center}
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.8cm,
            ...
            \node[initial,state, accepting] (A)                    {$a_1$};
            \node[state, accepting]         (B) [right of=A] 	    {$a_2$};
            \node[state]         (C) [right of=B] 	    {$a_3$};
            
            \node[state]         (D) [below of=C] 	    {$a_4$};
            \node[state]         (E) [below of=B] 	    {$a_5$};
            
            \path 
            (A)  edge 				 node {$\word 0\io\9R$} (B)

            (B) edge [loop above]   node {$\stackedtight{\word 0\io \word 0R \\ \word 1\io\word 1R}$} (B)
            (B) edge                node {$\9 \io \9 L$} (C)
            
            (C) edge                node {$\word 1\io\9L$} (D)
            
            (D) edge [loop right]   node {$\stackedtight{\word 0\io \word 0L \\ \word 1\io\word 1L}$} (D)
            (D) edge                node {$\9 \io \9 R$} (E)
            
            (E) edge                node {$\word 0 \io \9 R$} (B)
            (E) edge                node {$\9 \io \9 L$} (A)
            ;
        \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}{Lösung}
    ...
    \begin{center}
        \begin{tikzpicture}[->,>=stealth,shorten >=1pt,auto,node distance=2.8cm,
            ...
            (E) edge                node[above] {$\word 1 \io \9R$} (C)
